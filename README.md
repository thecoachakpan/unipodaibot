...
1. **WhatsApp AI Background Worker (Render 24/7 Container)**
   ...
   - **Computer Vision Screenshot Diagnostic Engine**: Ingests image attachments (`image/jpeg`, `image/png`, `image/webp`) in RAM and uses Gemini's vision capabilities alongside session context.
   - **Group Chat Intelligence, Declarative Statement Guard & Privacy Guard**: Responds strictly to explicit tags/mentions (`@bot`, `!ask`), facilitator mentions, direct quote-replies to PodPal BOT, or fresh program questions. **Filters out declarative statements, peer comments, and group banter even if they contain program keywords.**
   ...
   - **Academic Integrity & Strict Assignment Boundary Guard**: Prevents the bot from completing assignments or solving technical task roadblocks (e.g., API key setup exercises, coding assignments). Guides participants strictly on task expectations, submission requirements, and portal navigation, referring technical task roadblocks to program facilitators and support (`unipods.regional@undp.org`).
   - **Direct Task Output & Dissatisfaction Escalation Protocol**: Responds directly to tasks (translations, reference messages, admin tags) without repetitive boilerplate outro promos. Welcomes feedback emoji reactions on satisfaction, asks clarifying follow-ups on vague dissatisfaction, and escalates persistent dissatisfaction by tagging relevant program admins (`@Gift`, `@Diane`, `@Charles`, `@Jeovaire`, `@Munira`) or providing support email (`unipods.regional@undp.org`).
