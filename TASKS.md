...
This document tracks the step-by-step implementation tasks and operational matrix for **PodPal BOT** (UniPods WhatsApp AI Assistant & Next.js Admin Dashboard with Supabase Auth).
...
- [x] **1.2 Dependencies & Configuration**
  ...
  - Install `@whiskeysockets/baileys`, `@google/genai`, `googleapis`, `dotenv`, `qrcode`.
...
- [x] **1.3 Database Schema & Render Deployment Files**
  - Create `schema.sql` for Supabase SQL Editor including `scheduled_reminders`, `dm_sessions`, `bot_config`, `whatsapp_auth`, `knowledge_entries`, `unresolved_queries`.
...
- [x] **2.2 Google Drive & Link Ingestion Module (`worker/googleDrive.js`, `worker/syncDriveCatalog.js`)**
...
- [x] **2.3 Admin Private DM Scheduling Engine (`worker/reminderScheduler.js`)**
  - Recognize verified Admin WhatsApp JIDs (Victor Akpan, Diane, Gift, Jeovaire, Munira, Charles Bolton).
...
- [x] **2.4 Baileys Engine, Gemini AI Pipeline & Smart DM Router (`worker/bot.js`)**
  ...
  - Implement **Per-Turn Multilingual Detection & Mid-Chat Language Switcher** (seamlessly switches between English, French, Arabic, Amharic, etc.).
  ...
  - Commands: `!links`, `!deadlines`, `!schedule`, `!reset`, `!delete`, `!start`, `!recap`.
...
- [x] **2.5 Grounded Knowledge Base Seeding (`worker/seedKnowledge.js`, `worker/addKnowledgeEntries.js`)**
...
- [x] **3.2 Admin Portal Interface (`app/admin/page.tsx`)**
  - Modern glassmorphic dark UI using Tailwind CSS & Lucide icons.
  - Master Kill Switch toggle (ACTIVE / OFFLINE).
  - Operational Scope selector (Both vs Private Only vs Group Deactivated).
...
## Phase 4: Documentation, Testing & Comprehensive Setup Guides
- [x] **4.1 Workspace Documentation & Maintainer Guides (`README.md`, `UniPods_AI_Assistant_Spec.md`)**
  - Document full system architecture, 2-tier Gemini AI fallback engine, Render zero-sleep self-ping heartbeat setup, UptimeRobot external pinging, Supabase setup, Google Drive setup, Admin DM scheduling, Smart DM routing, language switching, welcome persona, multi-user summary queue, message deletion, Web QR server (`/qr`, `/reset-qr`), `/health` metric server, Admin group recap (`!recap`), Render deployment, and Vercel hosting.
  - Create **Complete Step-by-Step Setup Guide** in `README.md` for running and maintaining the bot from any system (local execution, environment variables, Supabase SQL, Google Drive Service Account, Render 24/7 web container, Vercel).
  - Create **Admin & Maintainer Operating Manual** in `README.md` detailing admin DM commands (`!remind`, `!announce`, `!recap`, `!delete`, `!links`, `!reset`), `FACILITATOR_MAP` configuration, scope mode selection, session reset procedures via `/reset-qr`, KB maintenance, and troubleshooting.
...
- [x] **5.3 `!recap` Command Implementation (`worker/bot.js`)**
  ...
  - KB-based recap for "week/all": Pulls relevant entries via `getRelevantKnowledgeContext()` for historical context
